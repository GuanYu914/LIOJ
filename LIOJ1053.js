...
function solve (lines) {
  ...
  let nodesLleftInLayer = 1     // 一開始的層數為 1
  ...
  while (!rq.isEmpty()) {
    ...
    for (let i = 0; i < 4; i++) {           // 開始找鄰近的點
      ...
      nodeINextLayer++ // 紀錄這層所有要走訪的點的數量
    } // 以上已完成走訪該層的一個點

    nodesLleftInLayer--                           // 將該層要走訪的點減一
    if (nodesLleftInLayer === 0) {                 // 若此層所有的點都走訪過了
      nodesLleftInLayer = nodeINextLayer    // 將下一層的點加入要走訪的
      nodeINextLayer = 0                      // 將下一層該走訪的點歸零
